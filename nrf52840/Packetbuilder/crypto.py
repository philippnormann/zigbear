...
def zigbee_get_packet_header(aPacket):
    ...
    if ZigbeeAppDataPayload2 in aPacket:
        if data_len > 0:
            header = bytes(aPacket[ZigbeeAppDataPayload2])[:-data_len]
        else:
            header = bytes(aPacket[ZigbeeAppDataPayload2])
    else:
        if data_len > 0:
...
def zigbee_packet_encrypt(key, aPacket, text, extended_source):
    ...
    new_packet = aPacket.copy()
    new_packet[ZigbeeSecurityHeader2].nwk_seclevel = 5
